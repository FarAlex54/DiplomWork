...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
                ...
                .requestMatchers("/authentication","/registration","/error","/resources/**","/static/**","/css/**","/js/**","/img/**", "/font/**","/product","/product/info/{id}", "/product/search","/person_account", "/person_account/product/info/{id}", "/hello","/user/infoProduct","/user").permitAll()
                .anyRequest().hasAnyRole("USER","ADMIN") // остальные страницы доступны всем ролям
                .and() //соединитель
//                .formLogin().loginPage("/authentication") //перенаправление при заходе на защищеные страницы
                ...
                .defaultSuccessUrl("/person_account",true) // после успешной аутентификации отправить пользоватьеля на эту страницу
                .failureUrl("/authentication?error")//куда отправляется не авторизованный пользователь
                .and().logout().logoutUrl("/logout").logoutSuccessUrl("/product");
        return http.build();
